const input = `LLLLLLLLLLLLLLLLLLLLLLL.LLLLLLL.L.LLLLL.LLLL.LLLLLLLLL..LLL.LLLLLLLLLLLLLL.LLLLLLLLL.LLL.LLLLLL
...
let map = JSON.parse(JSON.stringify(ogMap));

console.log(map)
let anyChanges = true

function squareNewStateAndChange(r, c) {
    if (map[r][c] === '.') {
        return ['.', false];
    } else {
        neighbors = 0;
        for (let checkR = r - 1; checkR <= r + 1; checkR++) {
            for (let checkC = c - 1; checkC <= c + 1; checkC++) {
                if (map[checkR] && !(checkR === r && checkC === c) && map[checkR][checkC] === "#") {
...
function findNextSeat(r, c, stepR, stepC) {
    while(true) {
        r = r + stepR
        c = c + stepC
...
function squareNewStateAndChange2(r, c) {
    if (map[r][c] === '.') {
        return ['.', false];
    } else {
        neighbors = 0;
        for (let stepR = - 1; stepR <= 1; stepR++) {
            for (let stepC = - 1; stepC <= 1; stepC++) {
                if (!(stepR === 0 && stepC === 0) && findNextSeat(r, c, stepR, stepC) === '#') {
                    neighbors++
                }
            }
        }
...
while (anyChanges) {
    ...
    for (let r = 0; r < map.length; r++) {
        for (let c = 0; c < map[r].length; c++) {
            [state, change] = squareNewStateAndChange2(r, c);
            if (change) {
                anyChanges = true
                newMap[r][c] = state;
            }
...
let lCount = map.reduce((acc, row) => row.reduce((acc2, col) => col === '#' ? acc2 + 1 : acc2, 0) + acc, 0)

console.log(`Answer 1 ${lCount}`)
