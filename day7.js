...
for (const line of input.split("\n")) {
    ...
    const target = `${words[0]} ${words[1]}`
    bagMap[target] = []
    // No Other Bags
    if (words[4] == 'no' && words[5] === 'other')  {
        continue;
    }
    // Start after the word contain
    for (let i = 4; i < words.length; i += 4) {
        const count = words[i];
        const bag = `${words[i + 1]} ${words[i + 2]}`
        bagMap[target].push({count, bag});
    }
}

function canTakeGoldenBag(bag) {
    if (!bagMap[bag]) {
        return false;
    }

    for (const {bag: curBag} of bagMap[bag]) {
        if (curBag === 'shiny gold' || canTakeGoldenBag(curBag)) {
            return true;
        }
    }
...
let count = 0
for (bag of Object.keys(bagMap)) {
    if (canTakeGoldenBag(bag)) {
        count++
    }
}
...
function countTotalBags(bag) {
    ...
    let total = 1;
    for (const {bag: curBag, count} of bagMap[bag]) {
        total += countTotalBags(curBag) * count;
    }

    return total;
}
// Subtract 1 to not count the Shiny Gold bag 
console.log(`answer 2 ${countTotalBags('shiny gold') - 1}`)