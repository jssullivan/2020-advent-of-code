...
validChecks = [
    (passport) => passport['byr'] >= 1920 && passport['byr'] <= 2002,
    (passport) => passport['iyr'] >= 2010 && passport['iyr'] <= 2020,
    (passport) => passport['eyr'] >= 2020 && passport['eyr'] <= 2030,
    (passport) => {
        const heightInt = Number.parseInt(passport['hgt'])
    ...
    (passport) => passport['hcl'].match(/^#[a-fA-F0-9]{6}$/i),
    (passport) => ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(passport['ecl']),
    (passport) => passport['pid'].length === 9
]
...
function passportValidator(isPart1) {
    const passportArray = input.split('\n\n');
    let totalValid = 0;
    for (const passport of passportArray) {
        const passportObj = {};
        const passportParts = passport.replace(/\n/g, " ").split(' ')
        for (part of passportParts) {
